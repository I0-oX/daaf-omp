...
└─ Understanding the current agent landscape before adding to it
    └─ Read: .omp/agents/README.md (Agent Index + "Commonly Confused Pairs")
```
...
3. **Identify similar agents** — read `.omp/agents/README.md` (Agent Index + "Commonly Confused Pairs") to find the 1-3 most similar existing agents. You MUST differentiate from these in your Core Distinction table.
...
4. Write the agent file to `.omp/agents/[agent-name].md` following the template exactly
...
grep -l "agent-name" .omp/agents/README.md AGENTS.md README.md

# 2. Cross-agent consistency (run for new agent file)
grep -c "HIGH.*MEDIUM.*LOW\|BLOCKER.*WARNING.*INFO\|Learning Signal\|STOP Conditions" .omp/agents/[agent-name].md

# 3. Verify agent count matches actual count
ls .omp/agents/*.md | grep -v README | grep -v _revised | wc -l
...
Consult `.omp/agents/README.md` for the authoritative Agent Index with:
...
| `.omp/agents/README.md` | The single source of truth for the agent landscape — read during Phase 1 |
...
# Multiple skills (YAML block list — matches official OMP docs)
skills:
  - data-scientist
  - polars
```
...
- **File:** `.omp/agents/[lowercase-hyphenated].md`
