...
- See `agent_reference/AGENT_TEMPLATE.md` > "Model Field" and `.omp/skills/daaf-orchestrator/SKILL.md` > "Model Selection for Subagent Dispatch".
...
Every agent's output must let the reader distinguish **observed facts from inference** (this mirrors AGENTS.md § Execution Philosophy > "Evidence-graded reporting" — the agent-level expression of that universal principle):
...
```bash
AGENT_FILE=".omp/agents/[agent-name].md"
