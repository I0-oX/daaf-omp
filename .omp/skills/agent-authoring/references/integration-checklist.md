...
### 1. `.omp/agents/README.md` — Agent Index table
...
### 2. `.omp/agents/README.md` — "When to Use" section
...
### 3. `.omp/agents/README.md` — Agent Coordination Matrix
...
**What to add:** Update agent count and add row to agent table if one exists. This is the project root `README.md`, distinct from `.omp/agents/README.md`.
...
**Section:** "Skill-to-Stage Mapping" (in `.omp/skills/daaf-orchestrator/references/full-pipeline-mode.md`)
...
**Section:** "Core Workflow Overview" (in `.omp/skills/daaf-orchestrator/references/full-pipeline-mode.md`)
...
**Section:** "Handoff Specifications" (in `.omp/skills/daaf-orchestrator/references/full-pipeline-mode.md`)
...
**Section:** "Stage Gates (Cannot Proceed Without)" (in `.omp/skills/daaf-orchestrator/references/full-pipeline-mode.md`)
...
### 12. `.omp/agents/README.md` — Orchestration Flow diagram
...
- **Configuration files:** `docker-compose.yml`, `.omp/config.yml`, `.omp/config.yml`, `.pre-commit-config.yaml`
- **Research project outputs:** Anything in `research/` directory
- **Existing skill files:** Anything in `.omp/skills/` (unless the agent needs a companion skill)
...
for f in .omp/agents/README.md; do
  echo "$f: $(grep -c "$AGENT_NAME" "$f") references"
done
```

**Expected:** .omp/agents/README.md has 3+ references (index, when-to-use, coordination matrix).

### 2. Agent Count Consistency

```bash
# Count actual agent files (excluding README, _revised, and this file)
ACTUAL=$(ls .omp/agents/*.md | grep -v README | grep -v _revised | wc -l)
...
```bash
# Verify no broken agent references in .omp/agents/README.md
# (Every agent mentioned in .omp/agents/README.md should have a file in .omp/agents/)
  name=$(echo "$agent" | tr -d '*')
  if [ -f ".omp/agents/${name}.md" ]; then
    echo "OK: $name"
  else
    echo "MISSING: $name"
  fi
...
### Tier 1: MANDATORY
- [ ] .omp/agents/README.md — Agent Index table (new row)
- [ ] .omp/agents/README.md — "When to Use" section (new subsection)
- [ ] .omp/agents/README.md — Agent Coordination Matrix (producer/consumer rows)
