...
**TARGET SKILL:**
- Name: {skill_name}
- Location: .omp/skills/{skill_name}/SKILL.md
...
5. **Cross-Dataset Discovery:** Glob for all `.omp/skills/*-data-source-*/SKILL.md` files.
...
7. Create reference files in .omp/skills/{skill_name}/references/:
...
8. Save draft to {project_dir}/output/skill_draft/ and final to .omp/skills/{skill_name}/
9. **Bundle Profiling Scripts:** Copy all executed profiling scripts (from STATE.md Profiling
   Progress table) to `.omp/skills/{skill_name}/scripts/`. These provide provenance and
...
- [ ] Profiling scripts bundled in .omp/skills/{skill_name}/scripts/ (excluding QA/cr scripts)
...
- [ ] Skill name conflict checked (no existing skill at `.omp/skills/{skill_name}/`)
- [ ] `{BASE_DIR}` path specified (for skill placement in `.omp/skills/`)
