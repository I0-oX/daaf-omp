...
Version suffixes follow the convention defined in `AGENTS.md` > "Version Control Protocol" (e.g., original → `a` → `b` → `c`).
...
These boundaries supplement the universal boundaries in `AGENTS.md` and `agent_reference/BOUNDARIES.md`.
...
**Version Suffix Convention:** See `AGENTS.md` > "Version Control Protocol" for the canonical convention (e.g., original → `a` → `b` → `c`).
