...
Data Onboarding mode is triggered when a user wants to profile raw data files and create a standalone data source skill. It produces a SKILL.md + reference files in `.omp/skills/` backed by a fully reproducible research project containing profiling scripts, QA reviews, and session state tracking.
...
│      ├─ Check for skill name conflict in .omp/skills/
...
│      ├─ Create reference files in .omp/skills/{skill-name}/references/
...
3. **Always check** for conflicts against existing skills in `.omp/skills/` before confirming.
...
> Before launching OMP, run this in your Docker container terminal:
> ```bash
...
**Stage DI-7 (Skill Authoring):** Does not follow this cycle. It has its own gate (GDI-7) and updates the Skill Authoring Status section of STATE.md directly. DI-7 includes skill file creation and template compliance validation. Skills are automatically discovered via YAML frontmatter once placed in `.omp/skills/`.
...
| Context utilization reaches ELEVATED (see AGENTS.md § Context Quality Curve for model-family thresholds) | Session Continuity → Context Snapshot |
...
**Skill Created:**
- Name: {skill-name}
- Location: {BASE_DIR}/.omp/skills/{skill-name}/
...
The skill is automatically discoverable via its YAML frontmatter and ready for use in future analyses. If you'd like to adjust the description or trigger conditions, edit the skill's frontmatter in `.omp/skills/{skill-name}/SKILL.md`.
...
These boundaries supplement the universal safety boundaries in `AGENTS.md`. This section is the canonical source for all Data Onboarding Mode-specific boundaries.
