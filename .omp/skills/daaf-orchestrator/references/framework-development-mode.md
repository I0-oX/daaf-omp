...
- [e.g., Created `.omp/agents/literature-reviewer.md` — new agent (482 lines)]
- [e.g., Updated `.omp/agents/README.md` — added to Agent Index, When to Use, Coordination Matrix]
...
**Context budget note:** Loading two skills at mode start consumes approximately 1,000-1,500 tokens of orchestrator context. Avoid loading additional skills directly in orchestrator context beyond these two — defer to subagents for any further skill loading. If context utilization reaches ELEVATED (see AGENTS.md § Context Quality Curve for model-family thresholds), consider whether direct advisory responses could be delegated to a framework-engineer dispatch instead.

---

## Workspace Setup

Framework Development typically modifies files in place (`.omp/skills/`, `.omp/agents/`, `agent_reference/`, `user_reference/`). No research project workspace is created unless the session produces artifacts that need a project home.
...
- Context utilization reaches ELEVATED (see AGENTS.md § Context Quality Curve for model-family thresholds)
...
- [e.g., Created `.omp/agents/literature-reviewer.md` (new agent)]
...
| Context reaches ELEVATED (see AGENTS.md § Context Quality Curve for model-family thresholds) | All sections (full checkpoint) |
...
These boundaries supplement the universal safety boundaries in `AGENTS.md`. See also `agent_reference/BOUNDARIES.md` > Framework Development Mode.
...
- Modifying AGENTS.md (universal authority document)
