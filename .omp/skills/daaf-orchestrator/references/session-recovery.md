...
| DI-2 (Project Setup) | Check project folder structure, verify STATE.md exists | `.omp/skills/daaf-orchestrator/references/full-pipeline-mode.md` (dispatch via OMP `task`; error recovery is OMP-native) |
...
| DI-7 (Skill Authoring) | Check if SKILL.md exists in `.omp/skills/{skill-name}/`; check Skill Authoring Status in STATE.md | STATE.md Skill Authoring Status + Interpretation Tracking |
