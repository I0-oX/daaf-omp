...
>
> **`.omp/agents/README.md`** provides agent behavioral specs and input/output contracts — consult when understanding an agent's capabilities, not for constructing invocation prompts.
...
> **DO NOT use OMP's built-in `EnterPlanMode` tool for this workflow.**
>
> This research system has its own planning protocol that is DIFFERENT from OMP's native plan mode:
>
> | Aspect | OMP plan mode | This Workflow |
> |--------|----------------------|---------------|
...
│      ├─ **WARNING:** DO NOT use OMP's built-in plan mode here!      │
...
The complete QA invocation workflow is defined in the **Stage 5-8 Composite Execution Pattern** below. See `.omp/agents/code-reviewer.md` for the QA protocol and `agent_reference/QA_CHECKPOINTS.md` for checkpoint definitions.
...
These checklists correspond to the input requirements defined in each agent's definition file (e.g., `.omp/agents/research-executor.md` > Inputs).
...
All code execution in Stages 5-8 MUST follow the file-first pattern (write → execute via wrapper → version on failure). See `AGENTS.md` > "Execution Philosophy" and `agent_reference/SCRIPT_EXECUTION_REFERENCE.md` for the complete file-first protocol.
...
- See also: `.omp/agents/data-ingest.md` Data Primacy table for the same hierarchy applied during data onboarding
...
See `agent_reference/QA_CHECKPOINTS.md` for complete definitions and `.omp/agents/code-reviewer.md` for the QA agent protocol.
...
| Context Utilization reaches ELEVATED (see AGENTS.md § Context Quality Curve for model-family thresholds) | Context Snapshot section |
...
These supplement the universal boundaries in `AGENTS.md` (Boundaries & Safety) and `agent_reference/BOUNDARIES.md`. See `BOUNDARIES.md` > Full Pipeline Mode for complete specifications.
...
The orchestrator receives actual context utilization via the `context-reporter` hook. See `AGENTS.md` > "Context & Session Health" for the complete threshold table and required actions.
...
On session end, the `archive-session.sh` hook automatically archives the full session transcript (JSONL + readable Markdown) to `.omp/logs/sessions/`. This provides a complete audit trail independent of STATE.md, useful for debugging cross-session issues or reviewing past decisions.
