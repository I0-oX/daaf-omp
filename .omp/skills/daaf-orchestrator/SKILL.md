...
**When detected**, check `AGENTS.md` § User Preferences. If still set to defaults
...
If the user confirms, update `AGENTS.md` § User Preferences:
...
`AGENTS.md` at session start and propagates the appropriate translation directive
...
the preference from `AGENTS.md` and silently propagate the directive — no need to
re-ask.
...
User Request
    ...
    │  Git, OMP), how it works, troubleshooting, or general
...
| **User Support** | "what is DAAF", "how does this work", "help me understand", "something's not working", "what can you do", "Docker", "Git", "OMP help" | Conversational guidance (no formal deliverables) | `user-support-mode.md` |
...
**User Support:**
> [Classification reasoning]. I'll read the user support mode workflow reference and load the core DAAF documentation so I can answer your questions thoroughly — how it works, what it can do, troubleshooting, best practices, anything about the system or the tools it runs on (Docker, Git, OMP). I can also look up official documentation online if needed. No formal outputs, just a conversation. If at any point you want to actually do something (run an analysis, look up data, etc.), I'll switch to the right mode. **Sound good?**
...
| "How long will this take?" | Describe remaining phases and checkpoints (no time estimates — per AGENTS.md) | — |
...
**Path convention:** **`{SKILL_REFS}`** = `{BASE_DIR}/.omp/skills/daaf-orchestrator/references`. Resolve `{BASE_DIR}` from your working directory (the project root where `AGENTS.md` resides).
...
- Use specialized agents for specific roles (see `.omp/agents/README.md` for the full agent index with inputs/outputs)
- Reference detailed protocols only when executing that protocol

### Agent vs. Skill Distinction

Skills provide **domain knowledge** ("What do I need to know?"). Agents define **behavioral protocols** ("How should I behave?"). See `.omp/agents/README.md` for the complete distinction table and agent catalog.
...
so the orchestrator can dispatch verification. See `AGENTS.md` § Execution
...
### Built-in OMP Skills (Not Part of DAAF)

The OMP harness bundles its own skills and slash commands (e.g.,
...
When `AGENTS.md` § User Preferences indicates a non-Python language background
...
DAAF uses **named agents** defined in `.omp/agents/`. When invoking a subagent, set `subagent_type` to the agent's `name` field from its frontmatter (e.g., `subagent_type: "research-executor"`). OMP automatically loads the agent's protocol file.
...
See `.omp/agents/README.md` for the complete agent index with key inputs and outputs.
...
Follow the context utilization thresholds defined in `AGENTS.md` > "Context & Session Health" > "Context Quality Curve". The orchestrator-specific relief mechanism is session restart via STATE.md (see `{SKILL_REFS}/session-recovery.md`).
