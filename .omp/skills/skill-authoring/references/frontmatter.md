...
> 2. **Display cap (1,536 chars combined):** The skill listing truncates the combined `description` + `when_to_use` text at 1,536 characters (raised from 250 in OMP) and warns at startup when truncation occurs.
...
>
> These numbers describe OMP behavior and are configurable (`maxSkillDescriptionChars` setting; `skillListingBudgetFraction` setting or `SLASH_COMMAND_TOOL_CHAR_BUDGET` env var for the aggregate budget). If observed behavior differs, verify against current OMP docs before assuming this reference is correct.
