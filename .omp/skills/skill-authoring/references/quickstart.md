...
OMP discovers skills from these paths (searched in order):

| Location | Scope | Path |
|----------|-------|------|
| Project config | Project | `.omp/skills/<name>/SKILL.md` |
| Global config | User | `~/.omp/skills/<name>/SKILL.md` |

For project-local paths, OMP walks up from the current directory to the git worktree root.
...
Create `.omp/skills/my-skill/SKILL.md`:
...
mkdir -p .omp/skills/my-skill/scripts

# For documentation
mkdir -p .omp/skills/my-skill/references

# For templates/assets
mkdir -p .omp/skills/my-skill/assets
```
