...
if [[ ! -f "$FLAG" ]]; then
    ...
    echo "You are interacting with a human user. You MUST IMMEDIATELY invoke the daaf-orchestrator skill (read skill://daaf-orchestrator) BEFORE doing any other work."

    # Check if this is the user's first-ever session
    ACTIVITY_LOG="$(pwd)/.omp/logs/activity.log"
    LINE_COUNT=0
