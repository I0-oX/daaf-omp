...
if [ "$SUBAGENT_TYPE" = "claude-code-guide" ]; then
  jq -n '{
    "hookSpecificOutput": {
      ...
      "permissionDecisionReason": "claude-code-guide is NOT permitted in this project. It runs on the Haiku model, which lacks the reasoning depth and nuance required for accurate OMP guidance. Instead, spawn a search-agent (subagent_type: search-agent) and instruct it to use web_search to browse the official OMP documentation at https://code.claude.com/docs/en/overview, navigating to and thoroughly reading the relevant pages for the question at hand. The search-agent inherits the main orchestrator model and has full access to web_search for retrieving documentation pages."
    }
  }'
fi
