// daaf-deny-claude-code-guide.ts — OMP extension: blocks the
// claude-code-guide agent from being spawned.
//
// Ported from .claude/hooks/deny-claude-code-guide.sh. The decision logic
// stays in the shell hook; this extension is a thin adapter that:
...
export default function (pi: ExtensionAPI): void {
  pi.setLabel("DAAF Deny Claude Guide");

  pi.on("tool_call", async (event) => {
    if (event.toolName !== "task") return;

    const payload = preToolUsePayload("Task", event.input);
    const result = await runHook("deny-claude-code-guide.sh", payload);

    if (result.code === 2) {
      return {
        block: true,
        reason: result.stderr.trim() || "Blocked by DAAF deny-claude-code-guide hook",
      };
    }
