...
If the orchestrator prompt indicates a non-technical target audience (policy, executive, public, or media): call the read skill:// with name `science-communication`. Apply its audience analysis, plain-language translation, and narrative framework guidance when drafting report sections in Step 6. If the audience is primarily technical/academic, skip this step — the `data-scientist` skill's methodology framing is sufficient.
