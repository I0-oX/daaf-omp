...
| Documentation website URL | Orchestrator Agent prompt | No | Fetch additional context via read in Part D |
...
When invoked, check the `profiling_part` parameter and execute the corresponding section below. For script templates and detailed profiling instructions, see `.omp/skills/daaf-orchestrator/references/WORKFLOW_PHASE_DO_PROFILING.md`.
...
1. **Research the API:** Use read to read API documentation. Use web_search if documentation URL is not provided. Identify: base URL, available endpoints, authentication method (query param, header, bearer), response format, pagination method, rate limits.
...
| **Process** | Execution patterns, error patterns | "read rate-limited after 5 codebook page fetches" |
...
**Invocation type:** `agent: "data-ingest"`
...
See `.omp/skills/daaf-orchestrator/references/WORKFLOW_PHASE_DO_PROFILING.md` for stage-specific invocation templates.
...
|-------|---------|-------------|
| `r-python-translation` | Orchestrator indicates user has R background | When profiling data for an R-background user, load this skill to annotate profiling scripts with R-equivalent comments. Load via read skill:// when directed. |
| `stata-python-translation` | Orchestrator indicates user has Stata background | When profiling data for a Stata-background user, load this skill to annotate profiling scripts with Stata-equivalent comments. Load via read skill:// when directed. |
