...
When debugging Stage 8 analysis failures: if the error traceback involves a specific modeling library (`pyfixest`, `statsmodels`, `linearmodels`, `geopandas`, or `scikit-learn`), call the read skill:// for that library. Each library's `gotchas.md` reference file documents common failure modes:
...
|-------|---------|-------------|
| `r-python-translation` | Orchestrator indicates user has R background | When debugging for an R-background user, load this skill to explain Python errors and fixes in R-equivalent terms. Load via read skill:// when directed. |
| `stata-python-translation` | Orchestrator indicates user has Stata background | When debugging for a Stata-background user, load this skill to explain Python errors and fixes in Stata-equivalent terms. Load via read skill:// when directed. |
