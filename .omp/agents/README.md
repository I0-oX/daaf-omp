...
- Web-capable research via web_search and read (unique among read-only agents)
...
2. **Author:** Create `.omp/agents/[agent-name].md` following `agent_reference/AGENT_TEMPLATE.md` (12 mandatory sections)
...
tools: [read, write, edit, bash, glob, grep, read]   # Explicit allowlist (OMP tool names). Omit for all.
...
- [ ] `.omp/agents/README.md` — Agent Index + "When to Use" section + Coordination Matrix
- [ ] `.omp/agents/README.md` — Agent catalog table (canonical Specialized Agents registry) + `full-pipeline-mode.md` > Skill-to-Stage Mapping (if stage-specific)
