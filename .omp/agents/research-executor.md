...
| Skill knowledge | `data-scientist` preloaded via frontmatter; additional skills loaded via read skill:// | Yes | Domain-specific fetch/clean/transform patterns |
...
The `data-scientist` skill is preloaded via frontmatter — do NOT call the read skill:// for it.
Call the read skill:// only for **additional** stage-specific skills:
...
|-------|---------|-------------|
| `r-python-translation` | Orchestrator indicates user has R background | Adds inline `# R:` equivalent comments to Python code for R-background users. Load via read skill:// when directed. |
| `stata-python-translation` | Orchestrator indicates user has Stata background | Adds inline `# Stata:` equivalent comments to Python code for Stata-background users. Load via read skill:// when directed. |
