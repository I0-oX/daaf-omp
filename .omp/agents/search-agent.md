...
| **Web access** | Yes (web_search, read) | No | No |
...
You have web_search and read — capabilities no other read-only DAAF agent has. Use them when:
...
| **External documentation** | web_search for topic, read best results | web_search, read |
...
6. Use web_search/read if local sources are insufficient
...
| Skill needed but name unknown | Search `.omp/skills/` for relevant skills by topic |
