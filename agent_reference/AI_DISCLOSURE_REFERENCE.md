...
|---|---|---|
| Justification for LLM choice | DAAF framework selection rationale | `[RESEARCHER]` — why DAAF was chosen over alternatives |
...
| **Session model ID** | Model driving the orchestrator/main session | From session metadata — record the model ID actually in use at session start, not a hardcoded example (e.g., "<session-model-id>[1m]") |
...
Record **resolved** model IDs (e.g., `<session-model-id>[1m]`) when known. The `opus`/`sonnet` values in agent frontmatter are aliases that resolve per provider and drift over time, so when a resolved ID is unavailable, record the alias plus the session date so the resolution can be reconstructed. Note the alt-provider case: users may remap these aliases via provider-specific model map env vars, in which case specialist models may not be standard-tier models and should be disclosed as whatever provider/model actually served the tier.

**Note on session transcripts:** DAAF automatically archives full session transcripts via OMP native session archiving. These transcripts are a powerful differentiator — most AI-assisted research cannot point to a complete record of the human-AI interaction. Researchers are encouraged to include these as supplementary material when submitting to journals, as they provide unparalleled transparency into the AI-assisted research process.
